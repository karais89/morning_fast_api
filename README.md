...
SQLAlchemy를 직접 사용하고 FastAPI와 함께 작동하는 데 어떤 종류의 플러그인도 필요하지 않기 때문에 데이터베이스 마이그레이션을 Alembic과 직접 통합 할 수 있습니다.

그리고 SQLAlchemy 및 SQLAlchemy 모델과 관련된 코드가 별도의 독립 파일에 있기 때문에 FastAPI, Pydantic 등을 설치하지 않고도 Alembic으로 마이그레이션을 수행 할 수 있습니다.

같은 방식으로 FastAPI와 관련이없는 코드의 다른 부분에서 동일한 SQLAlchemy 모델 및 유틸리티를 사용할 수 있습니다.

예를 들어 Celery, RQ 또는 ARQ를 사용하는 백그라운드 작업 작업자입니다.

## 모든 파일 검토

sql_app이라는 하위 디렉터리가 포함 된 my_super_project라는 디렉터리가 있어야합니다.

sql_app에는 다음 파일이 있어야합니다.
- sql_app / __ init__.py :는 빈 파일입니다.
- sql_app/database.py:
